# Feel Understood — The Dialogue System Source of Truth, V2

**What this is:** a deeper, richer replacement for the `THE DIALOGUE SYSTEM — YOUR SOURCE OF TRUTH:` block inside `buildCoachSystemPrompt()` in `index.html`.

**What changed from V1.** V1 was a table of contents — it told the coach the names of things. V2 tells it the *why*, the *tell-tale signs*, the *failure modes*, and the *words to use*. That's the difference between a coach who can list the four characteristics of dialogue and one who can hear a user's story and say "that sounds like the connecting broke down about four sentences in — you were both talking, but not to each other."

**Three things worth knowing before you drop it in:**

1. **It's written as plain text, not markdown.** No asterisks, no bold — the coach is voice-first and shouldn't be tempted to read formatting aloud. Hyphens and numbers only, matching the existing block.  
2. **It's long — roughly 5,000 words.** The prompt is rebuilt from scratch every turn, so this adds real tokens per request. Two options: (a) turn on prompt caching for the system block, which is the right answer since it's identical every turn; or (b) send Part A always and hold Part B back for a "deep dive" flag. Part A alone is about 60% of the length.  
3. **Part B is new territory** — it's the coach's working kit rather than the framework itself. It sits after `KEY PRINCIPLES` and before `HOW YOU INTERACT`.

---

## Part A — The System

Replaces everything between `THE DIALOGUE SYSTEM — YOUR SOURCE OF TRUTH:` and `HOW YOU INTERACT:`.
...
## Part B — The Coach's Working Kit

New material. Sits after Part A, before `HOW YOU INTERACT:`. This is the layer that turns knowledge into delivery — it's what stops the coach from answering every problem with a definition.

```
HOW TO USE THE SYSTEM AS A COACH
...
- **Turn on prompt caching** for the system block before shipping this — it's identical every turn, so it should be paid for once per session rather than 24 times.  
- **A/B it against V1** with a handful of beta trialists on the Helpline path. The thing to listen for is whether replies get more specific, or just longer. If they get longer, tighten the `VOICE & STYLE` block rather than cutting V2.  
- **Consider a third layer** later: a per-skill deep dive retrieved on demand (the CM1–CM10 / E1–E8 / U1–U8 journey in the skills map), so the coach can go deeper on one skill without carrying all of them in every request.
